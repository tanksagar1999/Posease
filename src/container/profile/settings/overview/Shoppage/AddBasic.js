...
const AddBasic = () => {
      ...
      if (data) {
        ...
        if (basicDetail) {
          ...
          setApiData({
            ...
            rs_symbol: basicDetail.rs_symbol
              ? basicDetail.rs_symbol
              : "India(₹)",
          });
          form.setFieldsValue({
            shop_name: basicDetail.shop_name,
  ...
  if (searchItems != "" && onClickList == false) {
    searchItemsList = countrySymbol.filter((value) => {
      return (
        value?.currency?.symbol
          ?.toLowerCase()
          .includes(searchItems.toLowerCase()) ||
        value.name.toLowerCase().includes(searchItems.toLowerCase())
      );
    });
  }
  const renderSymbol = (value) => {
    return (
      <div
        ...
        onClick={() => {
          setsearchItems(`${value.name} (${value.currency.symbol})`);
          setSelectSymbol(value.currency.symbol);
          setOnClickList(true);
        }}
      >
        <tr>
          <td>
            <p className="sp-product-name">{value.name}</p>
          </td>

          <td style={{ paddingLeft: "4px" }}>({value.currency.symbol})</td>
        </tr>
      </div>
    );
  };
                ...
                >
                  <BasicFormWrapper>
                    ...
                    >
                      ...
                      >
                        {countrySymbol.map((value) => (
                          <option
                            key={value.name}
                            value={`${value.name}(${value?.currency?.symbol})`}
                          >
                            {`${value.name} (${value.currency.symbol})`}
                          </option>
                        ))}
                      </Select>
                    </Form.Item>
                    ...
                    >
                      <Input
                        className="website-data social_input"
                        prefix={<GlobalOutlined style={{ fontSize: "20px" }} />}
                  ...
                  <Form.Item style={{ float: "right" }}>
                    <Button
                      ...
                      htmlType="submit"
                      size="small"
                      style={{ height: "inherit", padding: "8px 20px" }}
